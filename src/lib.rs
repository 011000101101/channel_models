...
pub fn calculate_paths_ce2r(x: f32, y: f32, z: f32, station_z: f32) -> Vec<(f32, f32, f32)> {
    ...
    let b: f64 = 2.0
        * ((m + 1.0) / (3.0 * m)).sqrt()
        * ((PI / 3.0) as f64
            + (3.0 * c * ((3.0 * m) / (m + 1.0).powi(3)).sqrt() / 2.0).acos() / 3.0)
            .cos();
    ...
    // lengths l_1 and l_2
    let l_1 =
        (abs_height_uav.powi(2) + KA.powi(2) - 2.0 * KA * abs_height_uav * (theta_1.cos())).sqrt();
    // let l_2 = r_2 - l_1;  // TODO
    let l_2 = (abs_height_station.powi(2) + KA.powi(2)
        - 2.0 * KA * abs_height_station * ((q - theta_1).cos()))
    .sqrt();
    // divergence factor D
    ...
    let phi: f64 = ((r_1.powi(2) + abs_height_uav.powi(2) - abs_height_station.powi(2))
        / (2.0 * r_1 * abs_height_uav))
        .acos();
...
fn generate_sample(parameters: (f32, f32, f32), dist: f32, mut rng: &mut ThreadRng) -> f32 {
    ...
    10.0_f32.powf(
        intercept
            + slope * (dist - 19_000.0).max(0.0)
            + Normal::new(0.0, std_dev).unwrap().sample(&mut rng),
    )
}
...
// 'step 1'
static INTERMITTENT_RAY_ORIGINS: Mutex<[IntermittentRayParameters; 7]> = Mutex::new(
    [IntermittentRayParameters {
        x: 0.,
        y: 0.,
        z: -1.0e10,
        is_on: false,
...
#[pyfunction]
pub fn calculate_paths_9ray_suburban(
    x: f32,
    y: f32,
    z: f32,
    station_z: f32,
    ...
    for k in 0..7 {
        if k == 0 || intermittent_ray_states[k - 1].is_on {
            let current_ray_parameters: &mut IntermittentRayParameters =
                &mut intermittent_ray_states[k];
            // 'step 4'
            let dist_to_ray_origin = ((current_ray_parameters.x - x).powi(2)
                + (current_ray_parameters.y - y).powi(2)
                + (current_ray_parameters.z - z).powi(2))
            .sqrt();
            if dist_to_ray_origin > current_ray_parameters.duration {
                // 'step 5'
                let on_probability =
                    generate_sample(DISTRIBUTION_ON_PROBABILITIES[k], dist, &mut rng);
                let on_sample = Uniform::new(0.0, 1.0).sample(&mut rng);
                current_ray_parameters.is_on = on_sample < on_probability;
                // 'step 6'
                current_ray_parameters.duration =
                    generate_sample(DISTRIBUTION_DURATION[k], dist, &mut rng);
            ...
            let additional_loss_db =
                (Normal::new(30.3, 4.1).unwrap().sample(&mut rng) as f32).max(0.0);
            ...
            // 'step 8'
            let excess_delay =
                generate_sample(DISTRIBUTION_EXCESS_DELAY[k], dist, &mut rng).max(0.0); // nanoseconds
            ...
            // println!("additional_loss_linear: {}", additional_loss_linear);
            paths.push((
                paths[0].0 * additional_loss_linear,
                excess_delay,
                phase_shift,
            ));
        }
